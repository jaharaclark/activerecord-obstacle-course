...
# ----------------------
  ...
  it '16. returns the names of users who ordered one specific item' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    # Solution goes here
    joined = OrderItem.where(item_id: @item_8.id).pluck(:order_id)
    found = Order.where(id: joined).pluck(:user_id)
    users = User.where(id: found).pluck(:name).sort
  ...
  it '17. returns the name of items associated with a specific order' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    # Solution goes here
    order_1 = Order.last.id
    joined = OrderItem.where(order_id: order_1).pluck(:item_id)
    names = Item.where(id: joined).pluck(:name)
  ...
  it '18. returns the names of items for a users order' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    # Solution goes here
    first = Order.where(user_id: @user_3.id).pluck(:id)
    second = OrderItem.where(order_id: first[2]).pluck(:item_id)
    items_for_user_3_third_order = Item.where(id: second).pluck(:name)
